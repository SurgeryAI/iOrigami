...
class OrigamiEngine: ObservableObject {
    ...
    func processFold(from: CGPoint, to: CGPoint, in view: SCNView) {
        ...
        guard let start = hitStart?.localCoordinates, let end = hitEnd?.localCoordinates else {
            return
        }
        ...
        if let minX = allX.min(), let maxX = allX.max(), let minY = allY.min(),
            let maxY = allY.max()
        {
    ...
    @objc private func updatePhysics() {
        ...
        // Calculate Median Z for Layer Separation
        let allZ = targetVertices.map { $0.z }
        ...
        for i in 0..<vertices.count {
            ...
            if isPuffed {
                // --- NEW ALGORITHM: Hinged Separation + Pneumatic Bulge ---

                // A. Hinged Layer Separation
                ...
                // B. Pneumatic Bulge
                // Simulate air pressure expanding the paper spherically.
                // Strongest at center, weakest at edges.
                let dist = sqrt(
                    currentTarget.x * currentTarget.x + currentTarget.y * currentTarget.y)
                let maxDist = paperSize * 0.6
                let bulgeFalloff = max(0, (maxDist - dist) / maxDist)
                let bulgeStrength: Float = 1.5 * bulgeFalloff * breathing

                let zDirection: Float = currentTarget.z >= medianZ ? 1.0 : -1.0
                let bulgeOffset = bulgeStrength * zDirection

                // Apply Forces
                currentTarget.z += separationOffset + bulgeOffset

                // Slight XY expansion to simulate paper tension
                currentTarget.x *= (1.0 + 0.05 * bulgeFalloff)
                currentTarget.y *= (1.0 + 0.05 * bulgeFalloff)
            }
